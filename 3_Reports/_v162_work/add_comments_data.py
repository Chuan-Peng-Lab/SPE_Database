# -*- coding: utf-8 -*-
"""Anchor text + comment body for every substantive v16.1 -> v16.2 change."""
...
COMMENTS = [
    ...
    ("Bootstrap Estimates of the Self-Prioritization Effect Under Nonmatching Conditions",
     "【Table 2】原表题为「Table / Summary of Liberal Approach Results」（无编号且只覆盖一种方法），"
     "现编号为 Table 2 并改名。8 行数值全部替换为 v2 重跑结果（mismatch_bootstrap_summary_v2.csv）："
     "保守法 N max 由 667/647 改为 1,563/1,494，自由法由 964/861/952/848 改为 3,604/3,192/3,524/3,114。"),

    ("Note. For RT, d = (Mean_Stranger",
     "【Table 2 注】补充「not reached」含义（CI 在全部样本量上都不排除 0），图引用由 Figure 4 改为 Figure 5。"),
    ...
    # NOTE: the bibliography below is a single Zotero field; Word refuses to anchor
    # comments inside it (the range collapses to the whole field). All reference
    # fixes are therefore collected into one comment on the "Reference" heading.
    ("Reference",
     "【参考文献·本次修正（共 6 项，按 Crossref 逐条核验）】\n"
     "1) 新增 Bogacz et al. (2006)：正文引用了但文献表里没有。Psychological Review, 113(4), 700–765, "
     "doi:10.1037/0033-295X.113.4.700。\n"
     "2) 原「Rose, H. (1991). Culture and the self… 1–30.」作者张冠李戴且缺期刊；实为 "
     "Markus, H. R., & Kitayama, S. (1991). Psychological Review, 98(2), 224–253, "
     "doi:10.1037/0033-295X.98.2.224。正文中的 (Rose, 1991) 已同步改为 (Markus & Kitayama, 1991)。\n"
     "3) Aron et al. (1992) 原缺期刊、页码写作 1–17；已补 Journal of Personality and Social Psychology, "
     "63(4), 596–612, doi:10.1037/0022-3514.63.4.596。\n"
     "4) Ghai (2024) 原缺期刊页码、DOI 有重复前缀、作者不全；已改为 Ghai, S., Forscher, P. S., & "
     "Hu, C.-P. (2024). Nature Human Behaviour, 8(6), 1053–1056, doi:10.1038/s41562-024-01902-y；"
     "正文引用改为 (Ghai et al., 2024)。\n"
     "5) Lin et al. (2023) 原缺期刊页码；已补 Psychological Bulletin, 149(7–8), 487–505, "
     "doi:10.1037/bul0000399。\n"
     "6) Sun et al. (2023) 原有两条重复条目（同一 DOI，标题与作者写法不同），已合并为一条，"
